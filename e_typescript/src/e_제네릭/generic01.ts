...
//! 제네릭(Generic, 일반적인)
//  : 한번 만든 코드로 여러 타입을 다루는 기술
//  >> 재사용 가능한 컴포넌트(코드단위)를 만드는 데 사용됨
//  >> 데이터 타입을 미리 지정하지 않고, 사용 시점에 데이터 타입을 지정하는 역할임

//? 제네릭의 필요성
//  : 코드 중복을 줄임, 타입에 대한 안정성을 보장함('컴파일' 시점에 타입을 체크하기 때문에)

//? 제네릭 기본 문법
//  : 사용할 컴포넌트(변수, 함수, 클래스 등)의 이름 뒤에 <> 작성
//  - 꺽쇠괄호 안에 타입 변수를 지정

// cf) 타입변수: 임의의 키워드 사용 (주로 T, U, V 등 대문자 한글자로 표현됨)
...
let pair1 = pair<number, string>(123, '안녕');
...
// let a = (x: number, y: number) => x + y; // 별로 권장하지 않는 방식
let myGenericCalss = new GenericClass<number>(0, (x, y) => x + y);
console.log(myGenericCalss.add(4, 5));
