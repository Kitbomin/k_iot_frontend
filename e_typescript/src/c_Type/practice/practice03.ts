...
document.addEventListener('DOMContentLoaded', () => {
  ...
  button.addEventListener('click', () => {
    ...
    // 3) JSON 문자열 생성
    const jsonData = JSON.stringify({name: username, age: 23});

    // 4) JSON을 객체로 변환 (타입 단언)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (element) {
    // 요소?
    // : 해당 요소가 있을 경우에 추후의 로직을 실행하는 (선택옵션)
    ...
    b?.addEventListener('click', () => {
      alert(element.innerText); // Element 내에서 "사용자에게 보옂지는" 텍스트 값을 반환
    });

    c?.addEventListener('click', () => {
      alert(element.textContent); // 태그와 상관없이 해당 요소가 가지는 텍스트 내용값을 그대로 반환함
    });
