...
class TaskManager<T> {
  // 1) 속성
  tasks: Task<T>[];
  nextId: number; // 다음 할 일에 할당할 고유 아이디 ID
  ...
  renderTasks(taskListId: string): void {
    ...
    this.tasks.forEach(task => {
      ...
      // : A요소 안에 B 요소를 삽입
      // - 내부의 첫번재 요소보다 전에 앞선 배치가 됨 (textContent 보다 앞선 배치를 하고싶음)
  ...
  updateTaskCount() {
    ...
    if (countElement) {
      countElement.textContent = `할일 개수: ${this.tasks.length}`;
    }
  }
}
