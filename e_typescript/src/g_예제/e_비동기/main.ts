...
//   : 50개의 데이터를 각 페이지에 4개씩 첨부함
//   : 50 === (4 * 12) + 2

//  2. 각 사진의 이름(title)은 사진 아래에 작성 (p 태그로)
...
let currentPage = 1; //현재 페이지 번호 추적 (기본값 1: 첫 페이지)
...
async function fetchPhotos(page: number) {
  try {
    ...
    //  page * photoPerPage: 끝 인덱스 (포함되지 않음)
