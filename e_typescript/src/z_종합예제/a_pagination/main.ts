...
const displayUsers = (users: User[]) => {
  ...
  if (userList) {
    ...
    users.forEach(user => {
      const userCard = createUserCard(user); // HTMLEliment 값ㅇ ㅣ반환됨
...
//! 비동기적으로 데이터를 가져와 각 페이지별 가트 생성 + 출력하는 함수
