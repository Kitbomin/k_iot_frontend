...
//! CSS Module
//  : 전역적인 CSS 적용은 아님 -> 컴포넌트 단위로 CSS를 작성하는 기능 (모듈화)
//  - 클래스 이름이 자동으로 로컬 범위(Scope)로 변환되는 방식임
//    > 각 컴포넌트에 대해 고유한 클래스 이름을 생성 + CSS 스타일 적용
//      ex) Button.tsx => Button.module.css 
//      ex) Card.tsx   => Card.module.css
//  - 클래스 이름 충돌의 상황이 없음. TypeScript 타입을 지원함
//  - React 컴포넌트 스타일 유지와 동시에 성능 / 유지보수성 / 가독성 이 좋은 CSS 코드를 작성 가능함.

//? 1. CSS Module 사용 방법
//     해당 컴포넌트에서 import 시 CSS 스타일이 객체로 인식됨
//   : 내부의 스타일 선택자를 객체의 속성처럼 사용하면 됨

//? 2. CSS Module 사용 팁
//  1) 각 컴포넌트별 폴더 구조 유지
//  2) 전역 스타일(variable.css)과 조합하여 많이 사용
//  3) 유지보수가 컴포넌트 수준에서는 완결적임
...
export const Button = () => {
  ...
  return (
    <button onClick={handleToggleStyle} className={`${styles.button} ${variant === 'secondary' ? styles.secondary : ""}`}>
...
//@ Header 컴포넌트: flex 사용
//  cf) flex는 한 줄에 정렬할 때 효율적임
//      : 헤더, 버튼 그룹, 푸터, 카드 내부 요소 등
//      > 주로 수평/수직 정렬, 리스트 아이템 배치에 사용됨 
export const Header = () => {
  return (
...
//  cf) 주로 대시보드, 상품 카드 리스트, 갤러리 형태에 많이 사용됨
//  : 가로 / 세로 모두 관리할 때 강력한 배치가 가능함
...
//  grid-template-columns - 열 개수 / 크기 정의 -> repeat(3, 1fr)
//  grid-template-rows    - 행 개수 / 크기 정의 -> auto 200px 1fr
//  gap                   - 아이템 간 간격      -> gap: 16px;
  ...
  return (
    ...
    <section className={gridStyles.dashboard}>
      ...
      <div style={{display: 'grid' , gridTemplateColumns: 'repeat(3,1fr)',}}>
        {/* 3열로 지정해놔서 더 열을 추가하려하면 아래로 흘러 내려감 */}
