...
import './truck-pages.css';

// 가게에 대한 상세 페이지
// : 가게 상세 정보 페이지
// - store를 통해 트럭 선택 / 예약 목록 호출을 제어
//   > 좌측 사이드 바에서 트럭을 선택(store.setSelectedTruckId + fetchReservations 호출)
//   > 중앙에서 예약 목록, 우측에 상세 컴포넌트를 렌더링 시켜줌
function TruckDetailPage() {
  const { selectedTruckId, setSelectedTruckId, fetchReservations, clearSelection } = useReservationStore();


  // 예시 트럭 목록 - 실제 앱에선 API로 받아옴
  const exampleTrucks = [
  ...
  // useEffect(() => {
  //   fetchTrucks();
  // +) 초기트럭 자동 선택 로직 추가 (owner입장: 제일 최근의 가게 / consumer입장: 제일 인기 많은 가게)
  // }, [fetchTrucks]);
