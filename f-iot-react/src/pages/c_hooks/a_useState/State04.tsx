...
/**
 *! useState를 사용한 이벤트 처리와 상태관리
 * ? 요구사항 정리
 *   1. 사용자 아이디, 비밀번호, 비밀번호 확인, 이메일 주소 입력
 *   
 *   2. 입력에 대한 유효성 검사
 *    : 모든 입력에 대한 필드창은 비워질 수 없음
 *    - 모두 입력해야함
 * 
 *   3. 상태 관리
 *    : 입력값들은 하나의 객체로 useState를 통해 관리됨 (formData)
 * 
 *   4. 폼 제출
 *    : 폼 제출 시 모든 입력값이 콘솔에 출력
 *    - 입력 조건을 만족하지 않은 경우 오류 메시지 출력
 *      > 오류 메시지도 상태 관리 
 * 
 */
...
function State04() {
  ...
  const handleSignUpSubmit = (e: React.FormEvent) => {
    ...
    //? 폼의 유효성 상태를 추적하는 변수
    let isValid = true; // 하나라도 유효하지 않으면 false로 지정할거임
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSignUpSubmit}>
        <div>
          <label> 아이디
            <input type="text" name='id' value={id} onChange={handleSignUpInputChange}/>
          </label>
          {/* error 객체에 해당 속성이 있으면 에러 출력 */}
          {/* JSX 안에서 태그 소괄호 생략 가능 -> 한 줄이라서 */}
          {error.id && <p style={{color: 'red'}}>{error.id}</p>}
        </div>

        <div>
          <label> 비밀번호
            <input type="text" name='password' value={password} onChange={handleSignUpInputChange}/>
          </label>
        ...
        <div>
          <label> 비밀번호 확인
            <input type="text" name='confirmPassword' value={confirmPassword} onChange={handleSignUpInputChange}/>
          </label>
        ...
        <div>
          <label> 이메일
            <input type="text" name='email' value={email} onChange={handleSignUpInputChange}/>
          </label>
