...
//  - 주로 UI에서 발생하는 이벤트에 반응해 상태 변화가 일어남
...
function State02() {
  ...
  const { id, password } = login; // 이렇게 구조분해 할당을 하기도 함
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // 매개변수로 전달받는 e의 target  >> 이벤트가 발생된 요소
    // > target 요소 내의 속성에 접근 가능

    // e.target
    // : name과 value 값을 추출
    // - name값은 상태 변수의 이름과 일치시키거나, 상태 변수 객체 내의 속성명과 일치시켜야함
    // - value값은 사용자로부터 입력받는 값음
    ...
    setLogin({
      ...
      [name]: value // name.value 이렇게 사용하지 않음
