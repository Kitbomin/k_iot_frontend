...
function TruckReservationDetail() {
  const { selectedTruckId, selectedTimeSlot, reservationList, setSelectedTimeSlot, fetchReservationById } = useReservationStore();
  ...
  const reservation = reservationList.find(reservation => reservation.timeSlot === selectedTimeSlot);

  if(!reservation) {
    return(
      <div className="panel">
        <h2 className="panel-title">예약 상세</h2>
        <p className="muted">선택한 예약 정보를 찾을 수 없어요</p>
        <div style={{marginTop: 12}}>
          <button type='button' className='small-btn' onClick={() => setSelectedTimeSlot(null)}>선택 해제</button>
        </div>
      </div>
    )
  }

  // 단건 새로고침 (store.fetchReservationById로 서버에서 최신정보 가져오기)
  const refreshReservation = async () => {
    ...
    try {
      ...
      setActionMessage("상세 정보를 가져오는 중 오류가 발생했습ㄴ디ㅏ.");
    } finally {
      setLoading(false);
    }
