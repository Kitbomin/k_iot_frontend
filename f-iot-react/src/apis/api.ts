...
// cf) process.env.VITE_API_BASE
//   : 환경변수
//   - REACT 앱에서 API 서버의 기본 URL 같은 설정값을 외부에서 주입받을 때 사용

export const api: AxiosInstance = axios.create({
  baseURL: API_BASE,
  timeout: 10000, //10초 타임아웃 - 요청이 10초 이상 걸릴 경우 자동으로 요청 취소 + 타임아웃 에러 발생
  headers: {
...
//  2) 자동 JSON 처리: 객체를 JSON으로 변환하거나 자동 파싱이 이루어짐
//  3) 응답 객체의 구조화: response.data 만 확인함
//  4) 인터셉터 지원: 요청/응답 가로채기 가능 (토큰/에러 처리 등)
//  5) Axios 인스턴스: 공통 설정을 모아 재사용이 가능함
...
//! 응답 인터셉터: 401 처리(Unauthorized: 자격증명 부족), 에러 포맷 통일
