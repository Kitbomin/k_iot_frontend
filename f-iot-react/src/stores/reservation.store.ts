...
export const useReservationStore = create<ReservationState>((set) => ({
  ...
  fetchReservations: async (truckId) => {
    // const reservations: ReservationsResponse = await getAllReservations(truckId);
  ...
  fetchReservationById: async (truckId, reservationId) => {
    try {
      ...
      // store의 단건을 리스트에 반영하거나 반환만 가능함
      // : 리스트에 해당 항목이 있으면 덮어쓰고, 없으면 앞에 추가
      // > set 설정함수는 내부에 콜백함수를 가짐 (해당 콜백함수의 매개변수는 상태의 최신값)
      set((state) => {
        // some() 메서드: 배열의 요소 중 조건을 만족하는 요소가 하나 이상인 경우 true
        // every() 메서드: 배열의 요소가 모두 해당 조건을 만족할 경우 true
  ...
  setSelectedTruckId: async (truckId) => set({ selectedTruckId: truckId }),

  setSelectedTimeSlot: async (timeSlot) => set({ selectedTimeSlot: timeSlot }),
