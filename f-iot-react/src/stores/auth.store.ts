...
interface AuthState {
  ...
  //@ 로딩 상태(로그인/ 로그아웃 요청 중 true / 갱신 중인지)
  isLoading: boolean;

  //@ 토큰 상태 -> 현재 보유 중인 Access Token 상태 (JWT)
  ...
  //^ === 액션(Action): 상태변경 또는 비즈니스 로직 수행 함수
  //@ 로그인 처리 (비동기, API 요청)
...
//! Zustand 스토어 생성
// create<AuthState>: AuthState 타입을 기반으로 스토어 구조 강제
  export const useAuthStore = create<AuthState>(
    ...
    set => ({
      //? 기본 상태 초기화
      user: null,         // 처음엔 로그인된 사용자가 없음
      accessToken: null,  // 처음엔 토큰도 없음
      isLoading: false,   // 로그인 갱신 로딩 상태도 아님
      error: null,        // 로그인을 안했으니 에러 메시지도 없음

      //? 액션 정의 (함수 정의)
      //@ setUser 함수: 상태에 user값을 직접 설정함
      setUser: (u) => set({user: u}),

      //@ setAccessToken 함수: 상태에 accessToken 값을 직접 설정
      setAccessToken: (token) => set({accessToken: token}),

      //@ login 함수: 로그인 API 요청(비동기함수) + 상태 업데이트
