//! === event === 
// : 웹 페이지에서 발생하는 대부분의 일(사건)을 의미함

//! === 이벤트 핸들링 (handle: 다루다) === 
// : 특정 이벤트에 반응해 특정 동작을 실행하는 것을 의미함 
...
//cf) 이벤트 핸들러 함수 정의
// > 이벤트에 반응해 실행될 함수 (콜백함수)
// > "랜덤 색상 생성 함수"
//    : rgb(r, g, b) - 0~255까지의 정수가 각각 지정되어있음


//? random 함수 정의: 0 ~ 255 까지의 랜덤 숫자 생성
function random(number) {
  //? Math.random
  //  : 0 이상 1 미만의 부동 소수점 난수 생성
  // EX) Math.random() * 256: 0 이상 256미만의 부동 소수점 난수

  //? Math.floor()
  //  : 괄호 안의 수를 내림하여 가장 가까운 정수를 생성하고 반환함

  return Math.floor(Math.random() * (number + 1));
} 

// EX) 0 이상 101 미만의 정수 생성
// : 0 부터 N까지의 정수 생성 === Math.random() * (N + 1);
...
//! !) HTML 이벤트 핸들러 속성 (프로퍼티)
//: HTML 요소에 직접 이벤트 핸들러 속성을 지정함
// - HTML 요소를 JS 객체로 가져와 핸들러 속성을 지정함


//? HTML 요소를 '참조(가져오는)' 방법
//? 웹문서.선택자질문('선택자');
//? > 선택자와 일치하는 문서 내 첫번 째 Element를 반환

const bgButton = document.querySelector("#bgChange");

// 전체를 선택하고싶다? -> querySelectorAll("선택자");
...
bgButton.onclick = function() {
  ...
  bgButton.style.border = '10px solid balck';
}


const keydownPara = document.querySelector("#keydown");

// +) 키보드 이벤트는 요소에 직접적인 등록이 아닌, 브라우저 전체 이벤트로 실행해야함
window.onkeydown = function() {
  keydownPara.textContent = "안녕하세요 반갑습니다.";
}

//! 인라인 이벤트 핸들러
//  : HTML 태그에 직접 onClick, onkeydown 등의 이벤트 속성을 사용해 함수 지정
// - 사용 권장 X (유지보수 및 코드 파싱 어려움)
// >> 리액트에서 사용하는 방식
...
// : 자바스크립트 표준 이벤트 모델
// : HTML 요소에 addEventListener 메서드를 사용해 이벤트를 등록해줄거임
// - 한 요소에 여러개의 이벤트 핸들러 등록이 가능함
...
buttons.forEach(btn => {
  ...
  // > HTML 요소에 해당 이벤트가 발생하면 콜백함수를 실행하겠단 의미
