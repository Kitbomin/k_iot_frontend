...
// return 결과값; (생략가능)
    //? rufrhkrkqt(return, 리턴값): 함수가 작업 완료 후 반환하는 데이터값 - return 키워드로 반환
// }
...
// > 호이스팅 적용이 되지 않음: 선언 이전에 호출이 불가능함
/**
...
const greet2 = function() {
  console.log("Hi Hello, 저는 함수 표현식이에요.");
} 
...
// 3) 화살표 함수
// : function 키워드 사용 X, 해당 키워드를 화살표(=>)로 대체함 (간결한 작성을 위함), 함수명 사용 X
// > 호이스팅 적용이 되지 않음: 선언 이전에 호출이 불가능함 
// +) 무조건 익명함수 형태로 작성해야함
...
console.log("== 화살표함수 선언 이전");

const greet3 = () => {
  console.log("곤니치와, 저는 화살표 함수에요");
}

greet3();

//? cf) 화살표함수는 매개변수가 1개인 경우 소괄호를, 함수 본문(구현부)이 단일문인 경우 중괄호 + return 키워드를 생략 가능함
...
// ! 5. 함수의 스코프(범위, 지역, scope)
// 1) 지역 변수 (local)
//  : 중괄호 내의 영역 (함수 내부)

// 2) 전역 변수 (global)
//  : 해당 파일(모듈) 내에서의 영역
//  - 해당 파일 내부에서 어디든지 접근이 가능함

let username = 'Ara';

function ara() {
  // 지역변수: 함수 내의 변수(+ 매개변수도 지역변수라 표현 가능)
  let job = 'developer';
...
// 지역변수는 해당 스코프를 벗어나면 사용이 불가능 해 아래와 같은 오류 발생
