...
async function fetchCustmerData() {
  try {
    // 실제 로직이 구현되는 블록
    let data = await fetch('https://jsonplaceholder.typicode.com/users/3'); // fetch(): Response 객체를 반환
...
// +) 실패 시 reject 값에 예외가 담김 - 네트워크 자체가 실패했을 대만 발생함 (404) 
...
// 4) headers: 응답의 헤더를 나타냄, 응답의 메타데이터나 특정 값을 포함함

// 5) body: 응답의 본문, 해당 데이터를 .json()으로 사용하여 data 추출
//    >> 응답의 기본값은 JSON 형식이기 때문